...
// 	     }
	...
	enum SipErrorCode errorCode = sip_error_unknown;
...
// 	}
	...
	else //not PJSIP_INV_STATE_DISCONNECTED
	{
		//先通知状态
			     ...
			     pjsip_rx_data *rdata)
{
	...
	else if (current_call !=call_id)
	{
		//new call incoming，just deny it
...
static pj_status_t app_init(void)
{
	...
	//init cmdline
	{
		//hwnd&server&id&user&pwd&stun&

		g_cmdparam.hwnd_ =(HWND)pj_strtol(&g_cmd_line);
		pj_size_t begin = pj_strcspn3(&g_cmd_line, "&",0);
		pj_size_t end = pj_strcspn3(&g_cmd_line, "&", begin + 1);
		memcpy(g_cmdparam.server_, g_cmd_line.ptr + begin+1, end - begin-1);

		begin = end;
		end = pj_strcspn3(&g_cmd_line, "&", begin + 1);
		memcpy(g_cmdparam.id_, g_cmd_line.ptr + begin + 1, end - begin-1);

		begin = end;
		end = pj_strcspn3(&g_cmd_line, "&", begin + 1);
		memcpy(g_cmdparam.user_, g_cmd_line.ptr + begin + 1, end - begin - 1);

		begin = end;
		end = pj_strcspn3(&g_cmd_line, "&", begin + 1);
		memcpy(g_cmdparam.pwd_, g_cmd_line.ptr + begin + 1, end - begin - 1);

		begin = end;
		end = pj_strcspn3(&g_cmd_line, "&", begin + 1);
		memcpy(g_cmdparam.stun_, g_cmd_line.ptr + begin + 1, end - begin - 1);

		//目前写死tcp和stun，未来可能要参数传递
		//use stun
		app_config.cfg.stun_srv_cnt = 1;
		app_config.cfg.stun_srv[0] = pj_str(g_cmdparam.stun_);
	...
	/* Adjust local account config based on pjsua app config */
	{
		...
		memset(id, 0, 128);
		strcpy(id, "<sip:");
		strcat(id, g_cmdparam.id_);
		strcat(id, "@");
		strcat(id, g_cmdparam.server_);
		strcat(id, ">");
		...
		memset(server, 0, 128);
		strcpy(server, "sip:");
		strcat(server, g_cmdparam.server_);
		strcat(server, ";transport=tcp");
		acc_cfg.reg_uri = pj_str(server);

		acc_cfg.cred_count = 1;
		acc_cfg.cred_info[0].scheme = pj_str("Digest");
		acc_cfg.cred_info[0].realm = pj_str("*");
		acc_cfg.cred_info[0].username = pj_str(g_cmdparam.user_);
		acc_cfg.cred_info[0].data_type = 0;
		acc_cfg.cred_info[0].data = pj_str(g_cmdparam.pwd_);

		acc_cfg.proxy_cnt = 1;
		acc_cfg.proxy[0] = pj_str(server);
